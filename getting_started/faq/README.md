...
## What is the meaning of SLURM job state codes displayed in output by the `squeue` command?
...
The system for pre- and post-processing for production projects at CSCS is Piz Daint. Please don't run directly on the login nodes, as they are a shared resource: you should submit your job on the SLURM partition `prepost`, adding the following line in your SLURM batch script:
```
...
You can find the documentation on the systems available at CSCS on the [CSCS web site](http://www.cscs.ch) and on the [CSCS User Portal](/). The different sections listed in the top menu of the pages will guide you through the facilities offered to the users community.
...
## Why do I get an error linking the MPI library on a Cray?
```
...
## How can I measure the performance of an application in flop/s?
